...
To compare the mpg means between automatic and manual transmissions.

### T-test

Assuming the a normal distribution of mtcars2 a ttest will be used to see if the mean MPG of auto VS manual transmission is significantly different .
...
```{r}
a2 <- lm(mpg~am_fac1 + hp + cyl, data = mtcars2)
summary(a2)$coef
```
...
```{r}
analysis <- aov(mpg ~ ., data = mtcars2) #I run ANOVA
summary(analysis) #this returns a summary containing relevant statistics
```


```{r}
anova(lm(mpg ~ am, data = mtcars2), lm(mpg ~ am + qsec, data = mtcars2), lm(mpg ~ am + wt + qsec, data = mtcars2))
```
...
```{r}
confint(a0)
as.numeric(confint(a0)[2, ][1])
as.numeric(confint(a0)[2, ][2])
confint(a3)
as.numeric(confint(a3)[2, ][1])
as.numeric(confint(a3)[2, ][2])
```
...
```{r fig.width = 10, fig.height = 10}
par(mfrow = c(2, 2))
plot(fitted(model.all), residuals(model.all), xlab = "Fitted values", ylab = "Residuals", main = "Residuals vs Fitted")
abline(h = 0, col = "red")
qqnorm(residuals(model.all))
qqline(residuals(model.all), col = "red")
plot(fitted(model.all), sqrt(abs(rstandard(model.all))), xlab = "Fitted values", ylab = "Square Root of Standardized Residuals", main = "Scale-Location")
```
